import { Button, FileDropzone, SelectInput, TextArea, TextInput } from "@jc-times/business-ui";
...
export function CurrentOperationPage() {
  ...
  return <div className={styles["current-page"]}>
    ...
    <section className={styles["operation-info"]} aria-label="当前工序信息">
      ...
      <div className={cx(styles["info-row"], styles["operation-row"])}><div className={styles["info-icon"]}><Target /></div><div className={styles["info-copy"]}><span>当前工序</span><strong>{current.operationName}</strong><Button variant="ghost" className={styles["note-button"]} onClick={() => setShowNote(true)}><MessageSquareText />查看工序备注<ChevronRight /></Button></div></div>
    ...
    <div className={styles["action-dock"]}>
      <Button variant="primary" className={styles["primary-action"]} disabled={actionLoading} onClick={primary.action}><PrimaryIcon />{actionLoading ? "正在处理..." : primary.text}</Button>
      {canSwitch && <Button variant="ghost" className={cx(styles["secondary-action"], styles["switch-action"])} disabled={actionLoading} onClick={() => setShowNext(true)}><ClipboardList />切换工序</Button>}
      {current.status !== "assigned" && current.status !== "completed" && <Button variant="ghost" className={styles["secondary-action"]} disabled={actionLoading} onClick={() => setShowComplete(true)}><Camera />结束并拍照</Button>}
    </div>
    {showNote && <BottomSheet footer={<Button variant="primary" className={styles["primary-button"]} onClick={() => setShowNote(false)}>我知道了</Button>} title="工序备注" onClose={() => setShowNote(false)}><div className={styles["note-content"]}><FileText /><p>{current.operationNote}</p></div></BottomSheet>}
...
function NoCurrentOperationPage({ candidates, onRefresh, onSelect, loading }: { candidates: OperationAssignment[]; onRefresh: () => void; onSelect: (assignment: OperationAssignment) => void; loading: boolean }) {
  if (!candidates.length) return <div className={styles["empty-state"]}><PackageCheck /><h1>当前没有待报工工序</h1><p>新的工序分配或领取后会显示在这里。</p><Button variant="primary" className={styles["primary-button"]} onClick={onRefresh}><RefreshCw />刷新工序</Button></div>;
  return <div className={cx(styles["standard-page"], styles["no-current-page"])}><PageHeader title="选择工序开始" subtitle="当前没有正在进行的工序，请先选择一道待开始工序" /><div className={styles["next-operation-list"]}>{candidates.map((item) => <Button variant="ghost" key={item.id} disabled={loading} onClick={() => onSelect(item)}><div><span>{item.orderNo}</span><em>{item.source === "self_claimed" ? "自主领取" : "后台分配"}</em></div><strong>{item.operationName}</strong><p>{item.productName} · {item.productCode}</p><p>{item.partName || item.partCode} · {item.partCode}</p><small><UsersRound />{item.collaborators.join(" / ")}</small><ChevronRight /></Button>)}</div><Button variant="ghost" className={cx(styles["ghost-button"], styles["refresh-wide"])} onClick={onRefresh}><RefreshCw />刷新工序</Button></div>;
}

function DayCompletedPage({ onRefresh }: { onRefresh: () => void }) {
  return <div className={styles["day-completed"]}><div className={styles["completion-check"]}><CheckCircle2 /></div><h1>今日工序已全部完成</h1><p>辛苦了！今天安排的报工任务均已完成。</p><div className={styles["completion-summary"]}><span>完成状态</span><strong>全部完成</strong></div><Button variant="ghost" className={cx(styles["ghost-button"], styles["day-completed-action"])} onClick={onRefresh}><RefreshCw />刷新工序</Button></div>;
}
...
function NextOperationSheet({ title, help, candidates, onClose, onSelect }: { title: string; help: string; candidates: OperationAssignment[]; onClose: () => void; onSelect: (assignment: OperationAssignment) => void }) {
  return <BottomSheet title={title} onClose={onClose}><p className={styles["sheet-help"]}>{help}</p><div className={styles["next-operation-list"]}>{candidates.map((item) => <Button variant="ghost" key={item.id} onClick={() => onSelect(item)}><div><span>{item.orderNo}</span><em>{new Date(item.plannedStart).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</em></div><strong>{item.operationName}</strong><p>{item.productName} · {item.productCode}</p><p>{item.partName || item.partCode} · {item.partCode}</p><small><UsersRound />{item.collaborators.join(" / ")}</small><small className={styles["operation-elapsed"]}><Clock3 />已用时长 {formatDuration(getSessionElapsedSeconds(item.session))}</small><ChevronRight /></Button>)}</div></BottomSheet>;
}
...
function PauseSheet({ onClose, onConfirm, loading }: { onClose: () => void; onConfirm: (reason?: string) => Promise<void>; loading: boolean }) {
  ...
  return <BottomSheet busy={loading} footer={<div className={styles["sheet-actions"]}><Button variant="ghost" className={styles["ghost-button"]} onClick={onClose}>取消</Button><Button variant="primary" className={styles["primary-button"]} disabled={loading} onClick={() => void onConfirm(reason).catch(() => undefined)}><Pause />确认暂停</Button></div>} title="确认暂停作业？" onClose={onClose}><p className={styles["sheet-help"]}>暂停期间不会计入本次作业工时。</p><label className={styles["field-label"]}>暂停原因（可选）<SelectInput value={reason} onChange={(e) => setReason(e.target.value)}><option>等待质检</option><option>设备调整</option><option>等待物料</option><option>临时离岗</option></SelectInput></label></BottomSheet>;
}
...
function CompleteSheet({ onClose, onConfirm, loading }: { onClose: () => void; onConfirm: (input: { photos: Array<{ name: string; url: string }>; completedQuantity?: number; note?: string }) => Promise<void>; loading: boolean }) {
  ...
  const onFiles = (files: readonly File[]) => {
    const result = selectImageFiles(files, photos.length);
  ...
  return <BottomSheet busy={loading || preparing} footer={<Button variant="primary" className={cx(styles["primary-button"], styles["submit-completion"])} disabled={!photos.length || loading || preparing} onClick={() => void submit()}><CheckCircle2 />{loading || preparing ? "正在提交..." : "确认完成报工"}</Button>} title="完成报工" onClose={onClose}><div className={styles["completion-step"]}><div className={styles["step-title"]}><span>1</span><strong>拍摄完工照片</strong><em>必填</em></div><FileDropzone className={styles["photo-picker"]} accept="image/*" inputProps={{ capture: "environment" }} multiple disabled={loading || preparing} onFilesSelected={onFiles}><Camera /><span>{photos.length ? "继续添加照片" : "拍照或选择照片"}</span></FileDropzone>{photos.length > 0 && <div className={styles["photo-grid"]}>{photos.map((photo, index) => <div key={photo.previewUrl}><img src={photo.previewUrl} alt={`完工照片 ${index + 1}`} /><Button variant="ghost" aria-label="删除照片" onClick={() => removePhoto(index)}><X /></Button></div>)}</div>}{fileError && <p className={styles["validation-hint"]}>{fileError}</p>}</div><div className={styles["completion-step"]}><div className={styles["step-title"]}><span>2</span><strong>补充完工信息</strong><small>选填</small></div><label className={styles["field-label"]}>本次完成数量<TextInput inputMode="numeric" value={quantity} onChange={(e) => setQuantity(e.target.value.replace(/\D/g, ""))} placeholder="请输入数量" /></label><label className={styles["field-label"]}>备注<TextArea value={note} onChange={(e) => setNote(e.target.value)} placeholder="填写异常、质量或交接说明" rows={3} /></label></div>{!photos.length && <p className={styles["validation-hint"]}>请至少添加一张完工照片</p>}</BottomSheet>;
}
