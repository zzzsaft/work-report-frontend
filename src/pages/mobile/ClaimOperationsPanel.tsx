...
export function ClaimOperationsPanel({
  ...
  return <section className={styles["claim-panel"]}>
    ...
    {view === "search" && <section className={cx(styles["claim-workspace"], styles["claim-search-workspace"])} aria-label="搜索结果">
      ...
      <label className={styles["claim-search"]}><Search /><input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="输入产品编号或工单号搜索" /><button disabled={loading || !keyword.trim()} onClick={() => void search()}>搜索</button></label>
      ...
      <div className={styles["claim-columns"]}>
        <div><div className={styles["claim-list-title"]}><h3>1 产品编号</h3>{!selectedProduct && productPagination.total > 0 && <span>共 {productPagination.total} 个</span>}</div>{selectedProduct ? <button key={selectedProduct.id} className={styles.selected} onClick={() => { setDismissedAutoProductId(selectedProduct.id); setSelectedProduct(null); setSelectedPart(null); setSearchPage(1); }}><strong>{selectedProduct.productCode}</strong><span>{selectedProduct.productName}</span><small>{selectedProduct.orderNo} · 剩余 {selectedProduct.remainingQuantity} 件</small><span className={styles["cancel-select"]}>点击取消选择</span></button> : products.map((item) => { const noPerm = item.hasPermission === false; return <button key={item.id} className={noPerm ? styles["no-permission"] : undefined} disabled={noPerm} onClick={() => { if (noPerm) return; setSelectedProduct(item); setSelectedPart(null); setSearchPage(1); setDismissedAutoPartId(null); void onLoadParts(item.id); }}><strong>{item.productCode}</strong><span>{item.productName}</span>{noPerm ? <small className={styles["no-permission-hint"]}>无权限</small> : <small>{item.orderNo} · 剩余 {item.remainingQuantity} 件</small>}</button>; })}{!selectedProduct && productPagination.total > productPagination.pageSize && <div className={styles["claim-pagination"]} aria-label="产品搜索分页"><button disabled={loading || productPagination.page <= 1} onClick={() => void loadProductPage(Math.max(1, productPagination.page - 1))} aria-label="上一页"><ChevronLeft />上一页</button><span>第 {productPagination.page} / {productPageCount} 页</span><button disabled={loading || !productPagination.hasMore} onClick={() => void loadProductPage(Math.min(productPageCount, productPagination.page + 1))} aria-label="下一页">下一页<ChevronRight /></button></div>}</div>
        <div><h3>2 部件编号</h3>{selectedPart ? <button key={selectedPart.id} className={styles.selected} onClick={() => { setDismissedAutoPartId(selectedPart.id); setSelectedPart(null); setSearchPage(1); }}><strong>{selectedPart.partCode}</strong><span>{selectedPart.partNo && `[${selectedPart.partNo}] `}{selectedPart.partName}</span><small>{selectedPart.operationCount} 道工序 · 剩余 {selectedPart.remainingQuantity} 件</small><span className={styles["cancel-select"]}>点击取消选择</span></button> : parts.map((item) => { const noPerm = item.hasPermission === false; return <button key={item.id} className={noPerm ? styles["no-permission"] : undefined} disabled={noPerm} onClick={() => { if (noPerm) return; setSelectedPart(item); setSearchPage(1); void onLoadOperations(item.id); }}><strong>{item.partCode}</strong><span>{item.partNo && `[${item.partNo}] `}{item.partName}</span>{noPerm ? <small className={styles["no-permission-hint"]}>无权限</small> : <small>{item.operationCount} 道工序 · 剩余 {item.remainingQuantity} 件</small>}</button>; })}</div>
      </div>
