import { Button, EmptyState, SegmentedControl } from "@jc-times/business-ui";
...
export function OperationsPage() {
  ...
  return <div className={styles["standard-page"]}>
    ...
    <section className={styles["claimed-section"]} aria-label="工序清单">
      <div className={styles["section-heading"]}><div><h2>我的工序</h2><p>v1 仅展示领取结果，报工状态流转将在后续版本开放。</p></div><Button variant="ghost" className={styles["ghost-button"]} disabled={assignmentsLoading} onClick={() => void loadAssignments()}><RefreshCw />刷新</Button></div>
      <div className={styles["history-filter"]} aria-label="工序清单时间筛选"><div><strong>时间范围</strong><span>共 {filteredAssignments.length} 条</span></div><SegmentedControl<OperationListRange> aria-label="工序清单时间筛选" value={range} onChange={setRange} options={operationListRangeOptions.map(([value, label]) => ({ value, label }))} variant="soft" /></div>
      {assignmentsLoading ? <LoadingState /> : <><div className={styles["operation-list"]}>{visibleAssignments.map((item) => <OperationCard key={item.id} item={item} removing={actionLoading} onRemove={() => void removeClaimedAssignment(item.id)} v1Status />)}{!visibleAssignments.length && <EmptyState className={styles["empty-inline"]} compact title={<>当前时间范围暂无已领取工序。</>} />}</div>{filteredAssignments.length > 0 && <div ref={loaderRef} className={styles["lazy-load-state"]}>{hasMore ? "继续下滑加载更多" : "已显示全部"}</div>}</>}
    </section>
  </div>;
}
