import { Alert, Button, EmptyState } from "@jc-times/business-ui";
...
export default function OrdersPage() {
  ...
  return <><AdminHeader title="工单管理" description="搜索工单，展开工序并手动分配生产人员" action={<SearchBox value={search} onChange={setSearch} placeholder="搜索工单、产品编号或名称" />} />{message && <Alert className={cx(styles["admin-message"])} tone={"info"} description={<>{message}</>} />}<section className={cx(styles["admin-panel"], styles["order-management-panel"])}>{loading ? <LoadingTable /> : <div className={cx(styles["order-management-list"])}>{filtered.map((item) => {
    ...
    return <article className={cx(styles["order-management-card"])} key={item.id}><Button variant="ghost" className={cx(styles["order-summary-row"])} onClick={() => void openOrder(item)} aria-expanded={isOpen}><div><strong>{item.orderNo}</strong><span>{item.productName} · {item.productCode}</span></div><div className={cx(styles["progress-cell"])}><span><i style={{ width: `${item.progress}%` }} /></span>{item.progress}%</div><AdminStatus status={item.status} />{canViewTeamOperations && <ChevronDown className={isOpen ? styles.rotated : undefined} />}</Button>{isOpen && canViewTeamOperations && <div className={cx(styles["order-operation-panel"])}>{panelLoading === item.id ? <LoadingTable /> : <><div className={cx(styles["part-tabs"])}>{orderParts.map((part) => <Button variant="ghost" key={part.id} className={activePart === part.id ? styles.active : undefined} onClick={() => void choosePart(part.id)}><strong>{part.partCode}</strong><span>{part.partNo && `[${part.partNo}] `}{part.partName}</span></Button>)}</div>{panelLoading === activePart ? <LoadingTable /> : activeOperations.length ? <div className={cx(styles["operation-assignment-list"])}>{activeOperations.map((operation) => <OperationAssignRow key={operation.id} operation={operation} canAssign={canAssignWorkers} onAssign={assignOperation} />)}</div> : <EmptyState className={cx(styles["empty-inline"])} compact title={<>当前工单暂无可分配工序。</>} />}</>}</div>}</article>;
  })}{!filtered.length && <EmptyState className={cx(styles["empty-inline"])} compact title={<>没有匹配的工单。</>} />}</div>}</section></>;
}
