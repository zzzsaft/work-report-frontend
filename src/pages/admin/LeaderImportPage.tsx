...
import { formatHours, hourAllocationTooltip, type LeaderImportDraft, type XftConfig, type XftHoursRow, type XftManualHoursDraft } from "@/domain/work-report";
...
function XftConfigPanel({ onSaved }: { onSaved: (config: XftConfig) => void }) {
  ...
  return <section className={cx(styles["admin-panel"], styles["xft-panel"])}><div className={cx(styles["panel-heading"])}><div><h2>薪福通配置</h2><p>{loading ? "正在读取配置" : config.hasAppSecret ? "密钥已配置，留空不会覆盖" : "请填写薪福通必填配置"}</p></div><Button variant="primary" className={cx(styles["admin-primary-action"])} disabled={saving || loading} onClick={() => void save()}><Save />保存配置</Button></div>{message && <Alert className={cx(styles["admin-message"])} tone={"info"} description={<>{message}</>} />}<div className={cx(styles["xft-config-grid"])}><label className={cx(styles["admin-field"])}>接口地址<TextInput value={config.host} onChange={(event) => update("host", event.target.value)} /></label><label className={cx(styles["admin-field"])}>应用 ID<TextInput value={config.appid} onChange={(event) => update("appid", event.target.value)} /></label><label className={cx(styles["admin-field"])}>应用密钥<TextInput type="password" value={config.appSecret || ""} placeholder={config.hasAppSecret ? "已保存，留空不变" : "必填"} onChange={(event) => update("appSecret", event.target.value)} /></label><label className={cx(styles["admin-field"])}>企业 ID<TextInput value={config.enterpriseId} onChange={(event) => update("enterpriseId", event.target.value)} /></label><label className={cx(styles["admin-field"])}>采集表名称<TextInput value={config.dataCollectionName} onChange={(event) => update("dataCollectionName", event.target.value)} /></label><label className={cx(styles["admin-field"])}>导入类型<TextInput value={config.importType} onChange={(event) => update("importType", event.target.value)} /></label><label className={cx(styles["admin-field"])}>薪资期间<TextInput value={config.salaryPeriod} onChange={(event) => update("salaryPeriod", event.target.value)} /></label><label className={cx(styles["admin-field"])}>工时字段<TextInput value={config.workHoursFieldKey} onChange={(event) => update("workHoursFieldKey", event.target.value)} /></label><label className={cx(styles["admin-field"])}>调用用户<TextInput value={config.defaultUserId} onChange={(event) => update("defaultUserId", event.target.value)} /></label><label className={cx(styles["admin-field"])}>平台用户<TextInput value={config.defaultPlatformUserId} onChange={(event) => update("defaultPlatformUserId", event.target.value)} /></label></div><div className={cx(styles["xft-toggle-row"])}><div><Checkbox label={<>校验空值</>} checked={config.isCheckEmpty} onChange={(event) => update("isCheckEmpty", event.target.checked)} /></div><div><Checkbox label={<>启用集成</>} checked={config.enabled} onChange={(event) => update("enabled", event.target.checked)} /></div></div></section>;
}
...
function XftHoursImportPanel({ salaryPeriod }: { salaryPeriod: string }) {
  ...
  const allocationSummary = useMemo(() => preview.reduce((summary, row) => ({
    applied: summary.applied + (row.appliedCount ?? (row.hourAllocation?.allocationApplied ? 1 : 0)),
    total: summary.total + (row.totalCount ?? (row.hourAllocation ? 1 : 0)),
  }), { applied: 0, total: 0 }), [preview]);
  ...
  return <section className={cx(styles["admin-panel"], styles["xft-panel"])}><div className={cx(styles["panel-heading"])}><div><h2>薪福通工时导入</h2><p>按期间汇总或手工填写员工工时后推送到采集表</p></div><label className={cx(styles["xft-period-input"])}>薪资期间<TextInput value={period} onChange={(event) => setPeriod(event.target.value)} /></label></div>{message && <Alert className={cx(styles["admin-message"])} tone={"info"} description={<>{message}</>} />}<div className={cx(styles["xft-import-grid"])}><section><div className={cx(styles["xft-section-head"])}><div><h3>按员工汇总</h3>{allocationSummary.total > 0 && <span className={cx(styles["allocation-summary"])} title={hourAllocationTooltip}><span>临时分摊</span>{allocationSummary.applied}/{allocationSummary.total} 条已按实际时长分摊</span>}</div><div><Button variant="ghost" className={cx(styles["table-action"])} disabled={loading} onClick={() => void previewHours()}><RefreshCw />预览</Button><Button variant="primary" className={cx(styles["admin-primary-action"])} disabled={loading} onClick={() => void importSummary()}><Upload />推送</Button></div></div><div className={cx(styles["table-wrap"], styles["compact"])}><ReportTable  columns={[{ title: "姓名" },
{ title: "工号" },
{ title: "工时" },
{ title: "说明" }]} rows={preview.map((row) => ({ id: String(`${row.staffNumber}-${row.lineId}`), cells: [<>{row.staffName}</>,
<>{row.staffNumber}</>,
<>{formatHours(row.hours)}</>,
<>{row.hourAllocation?.allocationTemporary ? <Badge className={cx(styles["allocation-tag"])} title={hourAllocationTooltip}>临时分摊</Badge> : "-"}</> ] }))} emptyTitle={"尚未预览"} /></div></section><section><div className={cx(styles["xft-section-head"])}><h3>手工导入</h3><Button variant="primary" className={cx(styles["admin-primary-action"])} disabled={loading || !manualDraftRows.length || manualErrors.length > 0} onClick={() => void importManual()}><Upload />推送手工表</Button></div>{manualErrors.length > 0 && <Alert className={cx(styles["admin-message"], styles["danger"])} tone={"danger"} description={<>{manualErrors.slice(0, 3).map((item) => `第${item.row}行 ${item.message}`).join("；")}</>} />}<div className={cx(styles["leader-import-sheet"], "ht-theme-main")}><HotTable data={manualRows} columns={[{ data: "staffName", width: 110 }, { data: "staffNumber", width: 120 }, { data: "hours", width: 80 }, { data: "identityNumber", width: 150 }, { data: "staffId", width: 110 }]} colHeaders={["姓名", "工号", "工时", "证件号", "员工序号"]} rowHeaders={true} height={240} width="100%" stretchH="all" minSpareRows={3} afterChange={updateManualRows} licenseKey="non-commercial-and-evaluation" /></div></section></div></section>;
}
...
export default function LeaderImportPage() {
  ...
  return (<>
    <AdminHeader title="小组长工序导入" description="直接从 Excel 粘贴或在单元格内编辑，格式错误会标红" action={canImportOperations && <Button variant="primary" className={cx(styles["admin-primary-action"])} disabled={submitting || !rows.length || errors.length > 0} onClick={() => void submit()}><Upload />确认导入</Button>} />
    ...
    <section className={cx(styles["admin-panel"], styles["import-panel"])}>
      <div className={cx(styles["import-header"])}>
        ...
        <div className={cx(styles["import-search"])}>
          ...
          {searchKeyword && <Button variant="ghost" className={cx(styles["table-action"])} onClick={() => setSearchKeyword("")}><X />清除</Button>}
        </div>
      </div>
