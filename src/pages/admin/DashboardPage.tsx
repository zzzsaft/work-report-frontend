import { MetricCard } from "@jc-times/business-ui";
...
function Kpi({ icon: Icon, label, value, unit, danger }: { icon: LucideIcon; label: string; value: string | number; unit: string; danger?: boolean }) { return <MetricCard icon={<Icon />} label={label} value={<>{value} <small>{unit}</small></>} tone={danger ? "danger" : "info"} />; }
