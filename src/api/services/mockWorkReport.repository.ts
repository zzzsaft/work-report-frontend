...
const initialDb = (scenario: "assigned" | "running" | "paused" = "running"): MockDb => ({
  ...
  reports: [
    { id: "report-001", orderNo: "WO-20260622-011", productCode: "CP-ZC-240622-03", productName: "传动轴", partNo: "01", partCode: "PART-SHAFT-001", partName: "主轴", operationCode: "OP-010", operationName: "粗加工 · 车削", operationNote: "切削前校验毛坯余量，切削后去毛刺并测量各段外圆尺寸。", ylpartnum: "YL-45#-001", yldescription: "45号圆钢，直径80mm", mfgcomment: "需调质处理后加工", cpNum: "CP-ZC-240622-03", cpDes: "原型号传动轴", operatorName: "张师傅", status: "completed", plannedQuantity: 8, claimedAt: "2026-06-22T08:00:00+08:00", estimatedHours: 8, allocatedHours: 4.75, originalEstimatedHours: 8, hourAllocation: { allocationTemporary: true, allocationApplied: true, allocationMethod: "actual_duration_ratio", allocationRatio: 0.59375, allocationBasisSeconds: 24300, allocationParticipantCount: 2 }, durationHours: 6.75, startedAt: "2026-06-22T08:12:00+08:00", completedAt: "2026-06-22T15:30:00+08:00", actualStartAt: "2026-06-22T08:12:00+08:00", actualEndAt: "2026-06-22T15:30:00+08:00", photos: [] },
    { id: "report-002", orderNo: "WO-20260623-021", productCode: "CP-FL-240623-02", productName: "连接法兰", partNo: "02", partCode: "PART-FLANGE-001", partName: "法兰盘", operationCode: "OP-040", operationName: "钻孔", operationNote: "使用M8丝锥，完工后逐件清理铁屑。", ylpartnum: "YL-FL-201", yldescription: "法兰毛坯件", mfgcomment: "", cpNum: "CP-FL-240623-02", cpDes: "原型号连接法兰", operatorName: "王师傅", status: "running", plannedQuantity: 12, claimedAt: "2026-06-23T08:30:00+08:00", estimatedHours: 4, allocatedHours: 4, originalEstimatedHours: 4, hourAllocation: { allocationTemporary: true, allocationApplied: false, allocationMethod: "original_estimated_hours", allocationParticipantCount: 2 }, durationHours: 3.4, startedAt: "2026-06-23T09:05:00+08:00", completedAt: undefined, actualStartAt: "2026-06-23T09:05:00+08:00", actualEndAt: undefined, photos: [] },
    { id: "report-003", orderNo: "WO-20260623-018", productCode: "CP-JSJ-240623-07", productName: "减速机外壳", partNo: "03", partCode: "PART-CASE-001", partName: "壳体主件", operationCode: "OP-030", operationName: "精加工 · 铣削", operationNote: "加工前确认夹具定位牢固；首件完成后检查孔距与表面粗糙度，发现毛刺立即停机反馈。", ylpartnum: "YL-CASE-102", yldescription: "铸铝壳体毛坯", mfgcomment: "注意砂眼缺陷", cpNum: "CP-JSJ-240623-07", cpDes: "原型号减速机外壳", operatorName: "李师傅", status: "paused", plannedQuantity: 6, claimedAt: "2026-06-23T08:00:00+08:00", estimatedHours: 7.5, durationHours: 2.1, startedAt: "2026-06-23T08:20:00+08:00", completedAt: undefined, actualStartAt: "2026-06-23T08:20:00+08:00", actualEndAt: undefined, photos: [] },
  ],
  exceptions: [
...
const createAssignmentFromPool = (operation: ClaimableOperation, source: OperationAssignment["source"], workerName = "张师傅"): OperationAssignment => ({
  ...
  ylpartnum: operation.ylpartnum,
  yldescription: operation.yldescription,
  mfgcomment: operation.mfgcomment,
  cpNum: operation.cpNum,
  cpDes: operation.cpDes,
...
export const mockWorkReportRepository: WorkReportRepository = {
  ...
  async claimOperation(operationId, input) {
    ...
    if (input?.quantity !== undefined) assignment.plannedQuantity = input.quantity;
  ...
  async getTeamOperationStats(company, teamName) {
    ...
    const db = load();
    const now = new Date();
    const teamByWorker = new Map(mockWorkers.map((w) => [w.name, w.teamName ?? "未分配班组"]));
    ...
    for (const r of db.reports) {
      ...
      if (!r.actualEndAt) continue;
      if (company && mockReportCompany(r) !== company) continue;
      const workerName = r.operatorName;
      const operationName = r.operationName || "未命名工序";
      const key = `${workerName}__${operationName}`;
      const planned = r.originalEstimatedHours ?? r.estimatedHours ?? 0;
      const actual = r.allocatedHours ?? r.estimatedHours ?? 0;
      const inMonth = isSameMonth(new Date(r.actualEndAt), now);
      let row = rows.get(key);
      if (!row) {
        row = {
          teamName: teamByWorker.get(workerName) ?? "未分配班组",
          workerId: workerName,
          workerName,
          operationName,
          totalPlannedHours: 0,
      ...
      row.totalPlannedHours += planned;
      row.totalActualHours += actual;
      if (inMonth) {
        row.monthPlannedHours += planned;
        row.monthActualHours += actual;
      }
    }
  ...
  async adminRemoveAssignment(assignmentId, reason) {
    ...
    const assignment = findAssignment(db, assignmentId);
    db.assignments = db.assignments.filter((item) => item.id !== assignmentId);
    db.reports.unshift({ id: `report-remove-${Date.now()}`, orderNo: assignment.orderNo, productCode: assignment.productCode || "", productName: assignment.productName, partNo: assignment.partNo || "", partCode: assignment.partCode || "", partName: assignment.partName || "", operationCode: assignment.operationCode || "", operationName: `${assignment.operationName}（后台移除：${reason}）`, operationNote: assignment.operationNote || "", ylpartnum: assignment.ylpartnum || "", yldescription: assignment.yldescription || "", mfgcomment: assignment.mfgcomment || "", cpNum: assignment.cpNum || "", cpDes: assignment.cpDes || "", operatorName: assignment.collaborators[0] || "张师傅", status: "cancelled", plannedQuantity: assignment.plannedQuantity, claimedAt: nowIso(), estimatedHours: 0, durationHours: 0, startedAt: nowIso(), completedAt: nowIso(), actualStartAt: assignment.actualStartAt, actualEndAt: assignment.actualEndAt, photos: [] });
    save(db);
  },
