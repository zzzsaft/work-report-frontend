...
v1 不需要实现任何报工状态流转。前端不会在 v1 入口调用当前工序、开始、暂停、恢复、完工、拍照或报工记录接口。
...
| `name` | string | 是 | 员工姓名 |
| `nameInitials` | string | 是 | 姓名首字母，建议小写，例如 `zsf` |
| `teamName` | string | 是 | 班组名称 |
| `activeAssignmentCount` | number | 是 | 当前有效工序数，用于分配时参考负载 |
...
| `plannedQuantity` | number | 是 | 计划数量 |
| `plannedStart` | string | 否 | 计划开始时间，ISO 8601；v1“查看最近”的“当日”筛选依赖该字段，缺失时前端只在“全部”中展示 |
...
v1 要求员工领取工序、查看工序清单和查看个人统计，不要求报工。以下接口是当前上线版本必须提供的业务接口。

### v1 页面与筛选口径

- `领取工序` 分为 `搜索领取` 和 `查看最近` 两个页内入口。
- `搜索领取`：前端按 `产品 -> 部件 -> 工序` 调用 `GET /claim/products`、`GET /claim/products/:productId/parts`、`GET /claim/parts/:partId/operations`。
- `查看最近`：前端用空 `keyword` 获取最近产品，再聚合产品下部件和工序；`当日` 只展示 `plannedStart` 为本地当天的工序，缺失 `plannedStart` 的工序只出现在 `全部`；`可领取` 只展示 `status = available`。
- `工序清单`：前端调用 `GET /assignments`，本地按 `今天`、`最近7天`、`最近一个月`、`全部` 筛选，并按 `claimedAt` 优先、其次 `plannedStart` 倒序展示；下滑加载只是前端分页渲染行为，接口仍返回数组。
- v1 移动端对已领取/已分配工序统一显示状态 `已领取`，不展示 `待开始`、`进行中`、`已暂停` 等报工状态标签。
- `我的统计`：前端调用 `GET /statistics/me?period=day|week|month`。

### V1-1. 获取当前用户工序列表

`GET /assignments`

用途：移动端“工序清单”。v1 前端展示当前用户已领取或已分配到自己的有效工序，并允许取消其中可取消的自领工序。
...
- 建议返回当前用户 `status` 非 `completed`、非 `cancelled` 的有效工序；如果后端返回历史完成数据，v1 当前页面会隐藏报工能力，只按前端筛选展示。
- 自领且可取消的工序：`source = self_claimed`、`status = assigned`、`canWorkerRemove = true`。
- 后台分配或不可取消工序：`canWorkerRemove = false`，前端只读展示。
- 必须提供 `plannedStart`；自领工序建议提供 `claimedAt`，用于工序清单排序和时间筛选。
- v1 不需要返回 `session`，也不会展示开始、暂停、恢复、完工等报工能力。
- 数组为空时返回 `[]`。
...
用途：员工按产品编号、产品名称或工单号搜索；也用于 v1“查看最近”聚合最近可以领取的工序。
...
- `plannedStart` 建议返回；这是 v1“查看最近”中 `当日/全部` 筛选的依据。真实接口暂未返回时，该工序不会被归入 `当日`。
...
用途：员工取消自己误领且未开始的自领工序。前端按钮文案为“取消领取”，接口仍沿用当前删除领取路径。

业务规则：

- assignment 必须属于当前用户。
- 只有 `source = self_claimed`、`status = assigned`、`canWorkerRemove = true` 时允许取消。
- 取消后回退对应 operation.claimedWorkers。
...
- v1 暂不按真实开始、暂停、完工记录计算工时；只要工序已领取或已分配给当前用户，即计入该用户统计。
- 统计工时使用 assignment 的标准/计划工时，优先取 `OperationAssignment.estimatedHours`；没有标准工时时按 `0` 处理，不推算真实耗时。
- `status = cancelled` 的 assignment 不计入统计；已领取、后台分配、小组长导入等未取消记录均计入。
- 日期归属优先按 `claimedAt`，没有 `claimedAt` 时按 `plannedStart`。
- `totalHours = regularHours = 已计入 assignment 的 estimatedHours 合计`；v1 暂无加班计算，`overtimeHours` 返回 `0`。
- `completedOperations` 在 v1 表示“已领取/已分配工序数”，不是完工数；后续 v2 真实报工口径再恢复为完工统计。
- `attendanceDays` 在 v1 表示统计周期内存在已计入工序的天数。
- `period = day`：返回当天领取/分配统计，`trend` 可返回 `[]`。
- `period = week`：返回自然周或最近 7 天统计，后端需保持口径稳定。
- `period = month`：返回自然月或最近一个月统计，后端需保持口径稳定。
- 没有数据时数值字段返回 `0`，`trend` 返回 `[]` 或补零趋势点，不要返回 `null`。

## v2 接口清单（升级预留）

v2 在 v1 基础上恢复完整报工和后台能力。下面保留完整接口说明，v1 当前不要求实现当前工序、开始、暂停、恢复、完工、出勤、后台报工记录等接口。
...
- assignment 必须属于当前用户。
- 只有 source = `self_claimed`、status = `assigned`、canWorkerRemove = true 时允许取消。
- 取消后回退 operation.claimedWorkers。
- 不满足条件返回 409。
...
`GET /admin/workers?keyword=zs&page=1&pageSize=20`

权限：`canAssignWorkers=true`

用途：后台工单管理面板给单道工序手动分配人员。前端支持按姓名、工号、班组和姓名首字母搜索，并按页懒加载。

Query：

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `keyword` | string | 否 | 搜索关键词；支持姓名、工号、班组、姓名首字母，例如 `zs` |
| `page` | number | 是 | 页码，从 1 开始 |
| `pageSize` | number | 是 | 每页数量，前端默认 5，后端可限制最大值 |
...
| `name` | varchar(100) | 姓名 |
| `name_initials` | varchar(32) | 姓名首字母，用于后台人员搜索 |
...
| `planned_quantity` | int | 计划数量 |
| `planned_start` | datetime | 计划开始时间，供 v1 最近工序“当日”筛选 |
...
2. v1 当前只实现领取、工序清单和个人统计；不要为了 v1 强行实现开始、暂停、完工、拍照或报工记录。
3. 返回字段必须完整，数组字段没有数据时返回 `[]`，不要返回 `null`。
4. `OperationAssignment` 是核心聚合对象，v1 至少要完整返回当前用户已领取/已分配且未取消、未完成的工序；可取消自领工序必须返回 `source = self_claimed`、`status = assigned`、`canWorkerRemove = true`。
